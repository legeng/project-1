		...
		function init(){
			...
			if(!(isset($_SESSION["isLogin"]) && $_SESSION["isLogin"]===1)){
				$this->redirect("login/index");
                            ...
                            if('/admin.php'!==$_SERVER['REQUEST_URI'] && '/admin.php/index/index' !== $_SERVER['REQUEST_URI'] && 'ajax' !== $_GET['type'] && 'ajax' !== $_POST['type']){
                                $_SESSION['origin_url'] = $_SERVER['REQUEST_URI']; 
                            }else{
                                $_SESSION['origin_url'] = $_SESSION['origin_url']; 
                            }
                        }

                        if(!(checkUserAuth($_SESSION["id"]))){
                            $this->primary("对不起，您的权限不足!" , 3);
                        }
